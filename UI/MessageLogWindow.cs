...
namespace SC_VSCode.UI
{
    ...
    public class MessageLogWindow : Window
    {
        ...
        public MessageLogWindow(int width, int height, string title) : base(width, height)
        {
            ...
            //Theme.WindowTheme.FillStyle.Background = DefaultBackground; - potentially change the background color of the window
            ...
            _messageScrollBar.ValueChanged += MessageScrollBar_ValueChanged; //subscribed to an event
            Add(_messageScrollBar);

            // enable mouse input
            UseMouse = true;

            // add child consoles to the window
            Children.Add(_messageConsole);
        }

        //always draw your windows in SadConsole!
